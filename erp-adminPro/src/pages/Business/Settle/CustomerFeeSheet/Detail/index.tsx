...
import { getCustomerFeeSheetById } from '@/api/business/settle';
...
const CustomerFeeSheetDetail: React.FC = () => {
    ...
    const loadData = async () => { if (!id) return; setLoading(true); try { const res: any = await getCustomerFeeSheetById(Number(id)); setSheet(res?.sheet || null); setDetails(res?.details || []); } catch (e) { console.error(e); } finally { setLoading(false); } };
    ...
    return (
        <div style={{ padding: '24px' }}>
            <Card title="客户费用单详情" extra={<Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/business/settle/customer/fee')}>返回</Button>}>
                <Spin spinning={loading}>
                    {sheet && (
                        <Descriptions bordered column={2}>
                            ...
                            <Descriptions.Item label="单据类型">{typeMap[sheet.sheetType]}</Descriptions.Item>
...
export default CustomerFeeSheetDetail;
