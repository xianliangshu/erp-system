...
import { getCustomerPreSheetById, addCustomerPreSheet, updateCustomerPreSheet } from '@/api/business/settle';
import { getCustomerList } from '@/api/basedata/customer';
import { getSettleInOutItemList } from '@/api/business/settle';

const CustomerPreSheetForm: React.FC = () => {
    ...
    const loadData = async () => {
        ...
        setLoading(true);
        try {
            const res: any = await getCustomerPreSheetById(Number(id));
            if (res?.sheet) {
                form.setFieldsValue({ customerId: res.sheet.customerId, description: res.sheet.description });
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            ...
            if (isEdit) { await updateCustomerPreSheet(data); message.success('修改成功'); }
            else { await addCustomerPreSheet(data); message.success('新增成功'); }
            navigate('/business/settle/customer/pre');
    ...
    return (
        <div style={{ padding: '24px' }}>
            <Card title={isEdit ? '编辑客户预收款单' : '新增客户预收款单'} extra={<Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/business/settle/customer/pre')}>返回</Button>} loading={loading}>
                ...
                <Card title="预收明细" size="small" style={{ marginTop: 16 }} extra={<Button type="dashed" icon={<PlusOutlined />} onClick={handleAddDetail}>添加</Button>}>
                ...
                <Space style={{ marginTop: 24 }}>
                    ...
                    <Button onClick={() => navigate('/business/settle/customer/pre')}>取消</Button>
                </Space>
            </Card>
        </div>
    );
...
export default CustomerPreSheetForm;
