...
import { getPreSheetPage, deletePreSheet, approvePreSheet, refusePreSheet } from '@/api/business/settle';
...
const PreSheetList: React.FC = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res: any = await getPreSheetPage({
                current,
                size: pageSize,
                code: searchCode,
    ...
    const columns = [
        ...
        { title: '创建时间', dataIndex: 'createTime', key: 'createTime', width: 180 },
        {
            title: '操作', key: 'action', width: 250, render: (_: any, record: any) => (
                <Space>
                    <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => navigate(`/business/settle/pre/detail/${record.id}`)}>查看</Button>
                    {record.status === 0 && (
                        <>
                            <Button type="link" size="small" icon={<EditOutlined />} onClick={() => navigate(`/business/settle/pre/edit/${record.id}`)}>编辑</Button>
    ...
    const handleApprove = async (id: number) => {
        try { await approvePreSheet(id); message.success('审核通过'); loadData(); } catch (e) { message.error('操作失败'); }
    };
    ...
    const handleRefuse = (id: number) => {
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                try { await refusePreSheet(id, reason); message.success('已拒绝'); loadData(); } catch (e) { message.error('操作失败'); }
            }
        });
    };
    ...
    const handleDelete = (record: any) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try { await deletePreSheet(record.id); message.success('删除成功'); loadData(); } catch (e) { message.error('删除失败'); }
            }
        });
    };
        ...
        <div style={{ padding: '24px' }}>
            <Card title="供应商预付款单">
                <Space style={{ marginBottom: 16 }} wrap>
                    ...
                    <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/business/settle/pre/add')}>新增</Button>
                </Space>
