...
import { getCustomerSettleSheetById, addCustomerSettleSheet, updateCustomerSettleSheet } from '@/api/business/settle';
import { getCustomerList } from '@/api/basedata/customer';
import dayjs from 'dayjs';

const CustomerSettleSheetForm: React.FC = () => {
    ...
    const loadData = async () => {
        ...
        setLoading(true);
        try {
            const res: any = await getCustomerSettleSheetById(Number(id));
            if (res?.sheet) {
                form.setFieldsValue({
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            ...
            if (isEdit) { await updateCustomerSettleSheet(data); message.success('修改成功'); }
            else { await addCustomerSettleSheet(data); message.success('新增成功'); }
            navigate('/business/settle/customer/sheet');
    ...
    return (
        <div style={{ padding: '24px' }}>
            <Card title={isEdit ? '编辑客户结算单' : '新增客户结算单'} extra={<Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/business/settle/customer/sheet')}>返回</Button>} loading={loading}>
                <Form form={form} layout="vertical" onFinish={onFinish} style={{ maxWidth: 800 }}>
                    ...
                    <Space style={{ marginTop: 24 }}>
                        ...
                        <Button onClick={() => navigate('/business/settle/customer/sheet')}>取消</Button>
                    </Space>
                </Form>
            </Card>
        </div>
...
export default CustomerSettleSheetForm;
