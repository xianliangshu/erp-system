...
import { getCustomerCheckSheetById } from '@/api/business/settle';
...
const CustomerCheckSheetDetail: React.FC = () => {
    ...
    const loadData = async () => { if (!id) return; setLoading(true); try { const res: any = await getCustomerCheckSheetById(Number(id)); setSheet(res?.sheet || null); } catch (e) { console.error(e); } finally { setLoading(false); } };
    ...
    return (
        <div style={{ padding: '24px' }}>
            <Card title="客户对账单详情" extra={<Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/business/settle/customer/check')}>返回</Button>}>
...
export default CustomerCheckSheetDetail;
