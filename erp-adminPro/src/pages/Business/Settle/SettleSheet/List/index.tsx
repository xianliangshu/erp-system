...
import { getSettleSheetPage, deleteSettleSheet, approveSettleSheet, refuseSettleSheet } from '@/api/business/settle';
...
const SettleSheetList: React.FC = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res: any = await getSettleSheetPage({ current, size: pageSize, code: searchCode, supplierId: searchSupplierId, status: searchStatus });
    ...
    const handleReset = () => { setSearchCode(''); setSearchSupplierId(undefined); setSearchStatus(undefined); setCurrent(1); loadData(); };
    ...
    const columns = [
        ...
        { title: '结算期间', key: 'period', width: 200, render: (_: any, r: any) => `${r.startDate || ''} ~ ${r.endDate || ''}` },
        { title: '应付金额', dataIndex: 'totalAmount', key: 'totalAmount', width: 120, render: (val: number) => `￥${val?.toFixed(2) || '0.00'}` },
        { title: '优惠金额', dataIndex: 'totalDiscountAmount', key: 'totalDiscountAmount', width: 100, render: (val: number) => `￥${val?.toFixed(2) || '0.00'}` },
        ...
        { title: '创建时间', dataIndex: 'createTime', key: 'createTime', width: 180 },
        {
            title: '操作', key: 'action', width: 250, render: (_: any, record: any) => (
                <Space>
                    <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => navigate(`/business/settle/sheet/detail/${record.id}`)}>查看</Button>
                    {record.status === 0 && (
                        <>
                            <Button type="link" size="small" icon={<EditOutlined />} onClick={() => navigate(`/business/settle/sheet/edit/${record.id}`)}>编辑</Button>
    ...
    const handleApprove = async (id: number) => { try { await approveSettleSheet(id); message.success('审核通过'); loadData(); } catch (e) { message.error('操作失败'); } };
    const handleRefuse = (id: number) => {
        Modal.confirm({
            title: '审核拒绝',
            content: <Input.TextArea id="refuseReason" rows={3} placeholder="请输入拒绝原因" />,
            onOk: async () => { const reason = (document.getElementById('refuseReason') as HTMLTextAreaElement).value; try { await refuseSettleSheet(id, reason); message.success('已拒绝'); loadData(); } catch (e) { message.error('操作失败'); } }
        });
    };
    const handleDelete = (record: any) => {
        Modal.confirm({
            title: '确认删除', icon: <ExclamationCircleOutlined />, content: `确定要删除单据 "${record.code}" 吗？`,
            onOk: async () => { try { await deleteSettleSheet(record.id); message.success('删除成功'); loadData(); } catch (e) { message.error('删除失败'); } }
        });
    };
        ...
        <div style={{ padding: '24px' }}>
            <Card title="供应商结算单">
                <Space style={{ marginBottom: 16 }} wrap>
                    ...
                    <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/business/settle/sheet/add')}>新增</Button>
                </Space>
...
export default SettleSheetList;
