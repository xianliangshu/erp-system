...
import { getPurchaseReturnPage, confirmPurchaseReturn, deletePurchaseReturn } from '@/api/business/purchaseReturn';
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, record: any) => (
                <Space size="small">
                    <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => navigate(`/business/purchase/return/detail/${record.id}`)}>
                        查看
                    </Button>
                    {record.status === 0 && (
                        <Popconfirm title="确定确认退货? 确认后将扣减库存" onConfirm={() => handleConfirm(record.id)}>
                            <Button type="link" size="small" icon={<CheckOutlined />}>
                                确认
                            </Button>
                        </Popconfirm>
                    )}
                    <Popconfirm title="确定删除此退货单?" onConfirm={() => handleDelete(record.id)}>
        ...
        <div style={{ padding: '24px' }}>
            <Card title="采购退货管理">
                <Space style={{ marginBottom: 16 }} wrap>
                    ...
                    <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/business/purchase/return/add')}>
                        新增退货
                    </Button>
                </Space>
