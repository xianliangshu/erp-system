...
import { getSaleReturnById } from '@/api/business/saleReturn';
...
const SaleReturnDetail: React.FC = () => {
    ...
    useEffect(() => { loadBasicData(); loadReturnData(); }, [id]);
    ...
    const loadReturnData = async () => {
        setLoading(true);
        try { const data: any = await getSaleReturnById(Number(id)); if (data) { setReturnData(data.saleReturn || data); setDetails(data.details || []); } }
        catch (e) { message.error('加载退货单数据失败'); } finally { setLoading(false); }
    };
    ...
    if (!returnData) return <div style={{ padding: '24px', textAlign: 'center' }}><p>退货单数据不存在</p><Button onClick={() => navigate(-1)}>返回</Button></div>;

    const statusInfo = statusMap[returnData.status] || { text: '未知', color: 'default' };

    return (
        <div style={{ padding: '24px' }}>
            <Card title={<Space><Button icon={<ArrowLeftOutlined />} onClick={() => navigate(-1)}>返回</Button><span>销售退货详情</span></Space>}
                extra={returnData.status === 0 && <Button type="primary" onClick={() => navigate(`/business/sale/return/edit/${id}`)}>编辑</Button>}>
                <Descriptions title="基本信息" bordered column={3}>
                    <Descriptions.Item label="退货单编号">{returnData.code || '-'}</Descriptions.Item>
                    <Descriptions.Item label="出库单编号">{returnData.deliveryCode || '-'}</Descriptions.Item>
                    <Descriptions.Item label="客户">{getCustomerName(returnData.customerId)}</Descriptions.Item>
