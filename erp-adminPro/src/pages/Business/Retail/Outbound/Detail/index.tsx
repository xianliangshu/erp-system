...
import { getRetailOutById } from '@/api/business/retail';
...
const RetailOutDetail: React.FC = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [whRes, custRes, matRes, detailRes]: any[] = await Promise.all([getWarehouseList(), getCustomerList(), getMaterialList(), getRetailOutById(Number(id))]);
            setWarehouses(whRes || []);
    ...
    const columns = [
        ...
        { title: '数量', dataIndex: 'outNum', key: 'outNum' },
    ...
    return (
        <div style={{ padding: '24px' }}>
            <Card title={<Space><Button icon={<ArrowLeftOutlined />} onClick={() => navigate(-1)}>返回</Button><span>零售出库单详情</span></Space>} extra={sheet.status === 0 && <Button type="primary" onClick={() => navigate(`/business/retail/out/edit/${id}`)}>编辑</Button>}>
...
export default RetailOutDetail;
