...
import { getRetailReturnPage, deleteRetailReturn, approveRetailReturn, refuseRetailReturn } from '@/api/business/retail';
...
const RetailReturnList: React.FC = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res: any = await getRetailReturnPage({
                current,
                size: pageSize,
                code: searchCode,
                scId: searchScId,
    ...
    const columns = [
        ...
        { title: '创建时间', dataIndex: 'createTime', key: 'createTime', width: 180 },
        {
            title: '操作', key: 'action', width: 250, render: (_: any, record: any) => (
                <Space>
                    <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => navigate(`/business/retail/return/detail/${record.id}`)}>查看</Button>
                    {record.status === 0 && (
                        <>
                            <Button type="link" size="small" icon={<EditOutlined />} onClick={() => navigate(`/business/retail/return/edit/${record.id}`)}>编辑</Button>
    ...
    const handleApprove = async (id: number) => {
        try { await approveRetailReturn(id); message.success('审核通过'); loadData(); } catch (e) { message.error('操作失败'); }
    };
    ...
    const handleRefuse = (id: number) => {
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                try { await refuseRetailReturn(id, reason); message.success('已拒绝'); loadData(); } catch (e) { message.error('操作失败'); }
            }
        });
    };
    ...
    const handleDelete = (record: any) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try { await deleteRetailReturn(record.id); message.success('删除成功'); loadData(); } catch (e) { message.error('删除失败'); }
            }
        });
    };
        ...
        <div style={{ padding: '24px' }}>
            <Card title="零售退货单">
                <Space style={{ marginBottom: 16 }} wrap>
                    ...
                    <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/business/retail/return/add')}>新增</Button>
                </Space>
...
export default RetailReturnList;
