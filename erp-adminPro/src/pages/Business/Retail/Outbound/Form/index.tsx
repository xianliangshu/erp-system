...
import { addRetailOut, updateRetailOut, getRetailOutById } from '@/api/business/retail';
...
const RetailOutForm: React.FC = () => {
    ...
    const loadDetail = async () => {
        ...
        setLoading(true);
        try {
            const res: any = await getRetailOutById(Number(id));
    ...
    const handleAddRow = () => {
        setDetails([...details, { key: Date.now(), productId: undefined, outNum: 1, taxPrice: 0, description: '' }]);
    };
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (details.some(d => !d.productId || !d.outNum)) { message.warning('请完善明细信息'); return; }

            setSubmitting(true);
            const data = { ...values, id: id ? Number(id) : undefined, details };
            if (isEdit) { await updateRetailOut(data); message.success('修改成功'); }
            else { await addRetailOut(data); message.success('新增成功'); }
            navigate('/business/retail/out');
        } catch (e) { console.error(e); }
    ...
    const columns = [
        ...
        {
            title: '数量', dataIndex: 'outNum', key: 'outNum', width: 120, render: (val: any, record: any) => (
                <InputNumber style={{ width: '100%' }} value={val} min={0.01} onChange={v => handleDetailChange(record.key, 'outNum', v)} />
            )
        },
        {
        ...
        {
            title: '金额', key: 'amount', width: 120, render: (_: any, record: any) => `￥${((record.outNum || 0) * (record.taxPrice || 0)).toFixed(2)}`
        },
    ...
    return (
        <div style={{ padding: '24px' }}>
            <Card title={isEdit ? '编辑零售出库单' : '新增零售出库单'} loading={loading} extra={<Space><Button icon={<ArrowLeftOutlined />} onClick={() => navigate(-1)}>返回</Button><Button type="primary" icon={<SaveOutlined />} loading={submitting} onClick={handleSubmit}>保存</Button></Space>}>
